                  ...
                  <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div class="bg-blue-50 rounded-xl p-5 border border-blue-200 hover:shadow-md transition-all duration-200 text-center group">
                      ...
                      <div class="text-3xl font-bold text-blue-600 mb-1">
                        {{ fincaDetalle?.estadisticas?.total_lotes ?? 0 }}
                      </div>
...
const loadFincaDetails = async (fincaId) => {
  if (!fincaId) return
  try {
    const fincaData = await loadFinca(fincaId)
    // Debug: verificar que las estadísticas estén presentes
    if (fincaData && fincaData.estadisticas) {
      console.log('Estadísticas recibidas:', fincaData.estadisticas)
    } else {
      console.warn('No se recibieron estadísticas en la respuesta:', fincaData)
    }
  } catch (error) {
