    ...
    def _check_directories(self):
        ...
        for dir_path in required_dirs:
            if not dir_path.exists():
                self.stdout.write(
                    self.style.WARNING(f"  ⚠ Directorio faltante: {dir_path}")
                )
    ...
    def _check_artifacts(self):
        ...
        for model_file in required_models:
            ...
            if model_path.exists():
                ...
                self.stdout.write(
                    self.style.WARNING(f"  ⚠ Modelo faltante: {model_file}")
                )
                all_found = False
        ...
        for scaler_file in required_scalers:
            ...
            if scaler_path.exists():
                ...
                self.stdout.write(
                    self.style.WARNING(f"  ⚠ Escalador faltante: {scaler_file}")
                )
                all_found = False
        ...
        if all_found:
            ...
            self.stdout.write(
                self.style.WARNING("  ⚠ Algunos artefactos faltan. Ejecutar entrenamiento primero.")
            )
    ...
    def _load_models(self):
        ...
        try:
            ...
            if success:
                ...
                self.stdout.write(
                    self.style.WARNING("  ⚠ Error cargando modelos")
                )
                
        except Exception as e:
            self.stdout.write(
                self.style.WARNING(f"  ⚠ Error cargando modelos: {e}")
            )
    
    def _show_api_info(self):
        """Muestra información de la API."""
        self.stdout.write("\n" + "="*50)
        self.stdout.write("INFORMACIÓN DE LA API")
