...
class ParametroDetalleSerializer(serializers.ModelSerializer):
    """Serializer para Parámetro con información del tema asociado."""
...
class ParametroCreateSerializer(serializers.ModelSerializer):
    """Serializer para crear un nuevo parámetro asociado a un tema."""

    class Meta:
        model = Parametro
        fields = ['tema', 'codigo', 'nombre', 'descripcion', 'activo']
        # unique_together=('tema','codigo') del modelo genera un
        # UniqueTogetherValidator que emite el error en non_field_errors.
